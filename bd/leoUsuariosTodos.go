...
//LeoUsuariosTodos Lee los usuarios del sistema
func LeoUsuariosTodos(ID string, page int64, search string, tipo string) ([]*models.Usuario, bool) {
	...
	for cur.Next(ctx) {
		...
		if r.UsuarioRelacionID == ID {
			incluir = false
			log.Println("aca los id son iguales")
		}
		fmt.Printf("encontrado: %t incluir: %t\n", encontrado, incluir)
		// son campos que blanqueo y que no se van a enviar
		if incluir == true {
			s.Password = ""
			s.Biografia = ""
			s.SitioWeb = ""
